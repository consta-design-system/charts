...
const Default = () => {
  const { data, risingFill, fallingFill, columnWidthRatio, labelMode } =
    getKnobs();
  ...
  return (
    <Waterfall
      ...
      renderer="svg"
      risingFill={
        vars.color.primary[`--color-bg-${risingFill}` as ColorSignature]
      }
      fallingFill={
        vars.color.primary[`--color-bg-${fallingFill}` as ColorSignature]
      }
      total={{
        style: {
