import { BulletExample, BulletExampleSingle, BulletExampleNoAxys, BulletExampleLabel } from './examples/BulletExample/BulletExample'
...
Вы можете поменять внешний вид подписи, которая выводится рядом с отрезком и показывает его значение. За это отвечает свойство `label`.

Например, можно поменять цвет подписи и переместить её внутрь отрезка. Так подпись заметнее и понятно, к какому именно отрезку она относится.
